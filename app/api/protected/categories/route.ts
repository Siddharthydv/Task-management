    ...
    try {
      ...
      return new Response(JSON.stringify(allCategories), { status: 200 });
        ...
        if(error instanceof Error)
            return new Response(JSON.stringify({ message: 'Error fetching categories', error: error.message }), { status: 500 });
    }
  }
...
export async function POST(req:Request) {
  ...
  try {
    ...
    const newCategory = await db.insert(categories).values({user_id,name,}).returning();

    return new Response(JSON.stringify(newCategory), { status: 201 });
  } catch (error) {
    if(error instanceof Error)
        return new Response(JSON.stringify({ message: 'Error creating category', error: error.message }), { status: 500 });
  }
}